...
#endif
{
    public class ListItemSet : IDataStoreItemSet
    {
        ...
        public bool NeedsSaving
        {
            get
            {
                foreach (ListItem li in this.items)
                {
                    if (li.LocalStatus == ItemLocalStatus.NewItem || li.LocalStatus == ItemLocalStatus.Update)
                    {
                        return true;
                    }
                }
        ...
        public ListItemSet(IDataStoreType list, Query query)
        {
            this.itemTypeList = (ItemTypeList)list;
            this.store = (ListStore)this.itemTypeList.Store;
        ...
        public IItem Create()
        {
            ListItem item = new ListItem(this.itemTypeList);

            this.store.NewItemsCreated++;

            item.SetId((-this.store.NewItemsCreated).ToString());
        ...
        private void AddManagersToItem(IItem item)
        {
            ...
            ((ListItem)item).Saving += ListItemSet_Saving;
        }

        private void ListItemSet_Saving(object sender, EventArgs e)
        {
        ...
        private void item_ItemDeleted(object sender, DataStoreItemEventArgs e)
        {
            ...
            if (this.autoSave)
            {
                ((ListItem)e.Item).Save(null, null);
            }
        ...
        public void BeginRetrieve(AsyncCallback callback, object state)
        {
            ...
            String endpoint = ((ItemTypeList)this.Type).GraphUrl + "/items/?expand=fields";
            ...
            if (isNew)
            {
                ...
                wr.AuthenticationRequired = false;
                wr.SetBearerToken(this.store.Graph.AccessToken);
...
{{
    ...
    {{
        ...
        if (idVal == null) 
        {{
            newe = new BL.Servicer.MSGraph.ListItem({2});
        }}
        else
        {{
        ...
        {{
            ...
            {{
                var fieldVal = fieldsVal[fi.get_name()];

                if (fieldVal != null)
                {{
                    newe.localSetValue(fiName, fieldVal);
                }}
            }}

            if (fiNameL == ""createdDateTime"")
            {{
                newe.setCreatedDateTime(newe.getValue(fiName));
            }}
            else if (fiNameL == ""lastModifiedDateTime"")
            {{
...
", results, this.itemTypeList.Fields, this.itemTypeList);
#else
